...
安装好以后，ByLoadTester 会自行启动，启动的时候，ByLoadTester会监听在某个端口上，获取性能统计数据。

所以初次启动，Windows会有防火墙提示是否允许程序监听端口，选择允许即可。


## 快速开始


ByLoadTester做性能测试通常分为如下几部：

### 第1步 创建项目

ByLoadTester 的界面如下图所示

![image](https://user-images.githubusercontent.com/36462795/61299037-f0d65700-a811-11e9-8fab-bc09f4458b22.png)

这个界面 就是让用户选择或者创建项目的。

ByLoadTester 项目 就是一个目录，里面包含了性能测试的定义文件。
...
```py
# 创建客户端     
client = ByHttpClient('127.0.0.1:80', # 目标地址:端口
                        timeout=10    # 超时时间，单位秒
                       ) 
...
```py
# 目标地址改为新浪服务器地址
client = ByHttpClient('hq.sinajs.cn', 
                        timeout=10
                       ) 
